...
class pascal_voc(object):
    def __init__(self, phase, rebuild=False):
        self.data_path = os.path.join(cfg.TL_PATH, 'YEIDO_TL_train_box1000')
    ...
    def get(self):
        ...
        labels = np.zeros((self.batch_size, self.cell_size, self.cell_size, 8))
        count = 0
    ...
    def load_labels(self):
        cache_file = os.path.join(self.cache_path, 'tl_' + self.phase + '_gt_labels.pkl')
        ...
        for index in range(1,1140):  #338 #3930
            label, num = self.load_tl_annotation(index)
            
            if index == 1:
                log_str2 = 'label{}.mat'.format(index)
                sio.savemat(log_str2, {'label':label})
    ...
    def load_tl_annotation(self, index):
        """
        ...
        annotation = np.genfromtxt(filename, delimiter='\t', dtype="str")
        #print filename
        #print len(annotation)
        
        if len(annotation) == 5: # 6:
            category = self.class_to_ind[annotation[0]]
            ...
            for box_index in range(len(annotation)):
                category = self.class_to_ind[annotation[box_index,0]]
                x_min = int(annotation[box_index,1])
                y_min = int(annotation[box_index,2])
                w = int(annotation[box_index,3])
                h = int(annotation[box_index,4])
                boxes = [x_min + w / 2.0, y_min + h / 2.0, w, h]
